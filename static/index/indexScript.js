...
function zapnut() {
    onDesign()
    // sendRequest('play', getSelectedStanica().getAttribute("data-url"), function() {
    post(getPostJson(
        ["url", getSelectedStanica().getAttribute("data-url")],
        "play",
        ["volume", document.getElementById("volumeController").value]), function() {
...
function vypnut() {
    offDesign()
    post(getPostJson('pause'), function(){
        offStatus()
        setVybranaText("")
    })
...
function loadChromecasts() {
    ...
    $("#cover").show();
    return $.get("/getChromecasts", function (data) {
...
function loadStanice() {
    //data = [{"name":<str>, "url:<str>"}, ...]
    $("#cover").show();
    return $.get("/getChannels", function (data) {
        staniceSetup(data)
        $("#cover").hide();
    })
}
